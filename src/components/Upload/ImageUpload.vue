...
<template>
  ...
  <div class="custom-upload-list">
    <!-- 已上传的图片列表 -->
    <div
      v-for="(path, index) in valFileList"
      :key="index"
...
import type { UploadRawFile, UploadUserFile, UploadFile } from 'element-plus';
...
watch(
  ...
  (newVal) => {
    ...
    const modelValue =
      typeof newVal === 'string' ? [newVal] : (newVal as string[]);
...
function handleBeforeUpload(file: UploadRawFile) {
  ...
  if (!fileExt) {
    ElMessage.warning(
      '上传图片格式错误,支持的文件类型:' + props.supportFileType.join(','),
    );
    return false;
  }
  // 给文件后缀名转换为小写
